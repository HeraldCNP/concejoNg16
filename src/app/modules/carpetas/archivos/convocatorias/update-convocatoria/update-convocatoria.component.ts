...
@Component({
  selector: 'app-update-convocatoria',
  templateUrl: './update-convocatoria.component.html',
  styleUrls: ['./update-convocatoria.component.scss']
})
export class UpdateConvocatoriaComponent implements OnInit {
    ...
    if (this.archivoForm.valid) {
      ...
      this.archivoService.updateArchivo(formData, this.idArchivo).subscribe((event: any) => {
        if (event.type === HttpEventType .UploadProgress) {
          ...
          this.router.navigate(['carpetas/listConvocatoria', this.archivo.idCarpeta._id]);
        }
      });
    }
    else {
