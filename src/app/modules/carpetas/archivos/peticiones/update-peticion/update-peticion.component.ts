...
@Component({
  selector: 'app-update-peticion',
  templateUrl: './update-peticion.component.html',
  styleUrls: ['./update-peticion.component.scss']
})
export class UpdatePeticionComponent implements OnInit {
    ...
    if (this.archivoForm.valid) {
      ...
      this.archivoService.updateArchivo(formData, this.idArchivo).subscribe((event: any) => {
        if (event.type === HttpEventType.UploadProgress) {
          ...
          this.router.navigate(['carpetas/listPeticion', this.archivo.idCarpeta._id]);
        }
      });
    }
    else {
