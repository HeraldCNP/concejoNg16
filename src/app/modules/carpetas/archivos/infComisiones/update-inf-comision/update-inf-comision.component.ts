...
@Component({
  selector: 'app-update-inf-comision',
  templateUrl: './update-inf-comision.component.html',
  styleUrls: ['./update-inf-comision.component.scss']
})
export class UpdateInfComisionComponent implements OnInit {
    ...
    if (this.archivoForm.valid) {
      ...
      this.archivoService.updateArchivo(formData, this.idArchivo).subscribe((event: any) => {
        if (event.type === HttpEventType.UploadProgress) {
          ...
          this.router.navigate(['carpetas/listInfComision', this.archivo.idCarpeta._id]);
        }
      });
    }
    else {
