...
@Component({
  selector: 'app-create-inf-comision',
  templateUrl: './create-inf-comision.component.html',
  styleUrls: ['./create-inf-comision.component.scss']
})
export class CreateInfComisionComponent implements OnInit {
    ...
    if (this.archivoForm.valid) {
      ...
      this.archivoService.createArchivo(formData).subscribe((event:any) => {
        if (event.type === HttpEventType.UploadProgress) {
          ...
          this.router.navigate(['carpetas/listInfComision', this.idCarpeta]);
        }
      });
    }
    else{
