...
@Component({
  selector: 'app-update-correspondencia',
  templateUrl: './update-correspondencia.component.html',
  styleUrls: ['./update-correspondencia.component.scss']
})
export class UpdateCorrespondenciaComponent implements OnInit {
    ...
    this.archivoService.getArchivoById(this.idArchivo).subscribe({
      next: (data: any) => {
        ...
        this.archivoForm.patchValue({
          ...
          origen: this.archivo?.origen,
          destino: this.archivo?.destino,
  ...
  onSubmit() {
    if (this.archivoForm.valid) {
      ...
      formData.append('destino', this.archivoForm.get('destino').value);
      ...
      this.archivoService.updateArchivo(formData, this.idArchivo).subscribe((event: any) => {
        if (event.type === HttpEventType.UploadProgress) {
          ...
          this.router.navigate(['carpetas/listCorrespondencia', this.archivo.idCarpeta._id]);
        }
      });
    }
