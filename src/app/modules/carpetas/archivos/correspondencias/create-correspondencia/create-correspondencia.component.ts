...
@Component({
  selector: 'app-create-correspondencia',
  templateUrl: './create-correspondencia.component.html',
  styleUrls: ['./create-correspondencia.component.scss']
})
export class CreateCorrespondenciaComponent implements OnInit {
    ...
    if (this.archivoForm.valid) {
      ...
      this.archivoService.createArchivo(formData).subscribe((event:any) => {
        if (event.type === HttpEventType.UploadProgress) {
          ...
          this.router.navigate(['carpetas/listCorrespondencia', this.idCarpeta]);
        }
      });
    }
    else{
