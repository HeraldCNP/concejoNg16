...
@Component({
  ...
  styleUrls: ['./update-minuta.component.scss']
})
export class UpdateMinutaComponent implements OnInit {
    ...
    if (this.archivoForm.valid) {
      ...
      this.archivoService.updateArchivo(formData, this.idArchivo).subscribe((event: any) => {
        if (event.type === HttpEventType.UploadProgress) {
          ...
          this.router.navigate(['carpetas/listMinuta', this.archivo.idCarpeta._id]);
        }
      });
    }
    else {
