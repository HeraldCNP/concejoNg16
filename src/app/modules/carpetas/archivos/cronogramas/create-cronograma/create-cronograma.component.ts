...
@Component({
  selector: 'app-create-cronograma',
  templateUrl: './create-cronograma.component.html',
  styleUrls: ['./create-cronograma.component.scss']
})
export class CreateCronogramaComponent implements OnInit{
    ...
    if (this.archivoForm.valid) {
      ...
      this.archivoService.createArchivo(formData).subscribe((event:any) => {
        if (event.type === HttpEventType.UploadProgress) {
          ...
          this.router.navigate(['carpetas/listCronograma', this.idCarpeta]);
        }
      });
    }
    else{
