...
@Component({
  selector: 'app-update-cronograma',
  templateUrl: './update-cronograma.component.html',
  styleUrls: ['./update-cronograma.component.scss']
})
export class UpdateCronogramaComponent implements OnInit {
    ...
    if (this.archivoForm.valid) {
      ...
      this.archivoService.updateArchivo(formData, this.idArchivo).subscribe((event: any) => {
        if (event.type === HttpEventType.UploadProgress) {
          ...
          this.router.navigate(['carpetas/listCronograma', this.archivo.idCarpeta._id]);
        }
      });
    }
