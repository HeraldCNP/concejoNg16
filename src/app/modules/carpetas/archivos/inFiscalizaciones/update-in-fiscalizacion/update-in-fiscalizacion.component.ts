...
@Component({
  selector: 'app-update-in-fiscalizacion',
  templateUrl: './update-in-fiscalizacion.component.html',
  styleUrls: ['./update-in-fiscalizacion.component.scss']
})
export class UpdateInFiscalizacionComponent implements OnInit {
    ...
    if (this.archivoForm.valid) {
      ...
      this.archivoService.updateArchivo(formData, this.idArchivo).subscribe((event: any) => {
        if (event.type === HttpEventType.UploadProgress) {
          ...
          this.router.navigate(['carpetas/listInFiscalizacion', this.archivo.idCarpeta._id]);
        }
      });
    }
